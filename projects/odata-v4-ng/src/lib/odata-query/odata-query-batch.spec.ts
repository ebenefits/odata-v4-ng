...
describe('ODataQueryBatch', () => {
    ...
    it('test execute', () => {
        const odataQueryBatch: ODataQueryBatch = new ODataQueryBatch(odataService, SERVICE_ROOT);
        const spy: Mock = vi.spyOn(odataService, 'post').mockReturnValue(null);
        odataQueryBatch.setBatchBoundary('batchBoundary');

        odataQueryBatch.execute();
        expect(vi.mocked(spy).mock.lastCall[0]).toEqual(odataQueryBatch);
        expect(vi.mocked(spy).mock.lastCall[1]).toEqual(odataQueryBatch.getBody());
        const httpOptionsIArg: HttpOptionsI = vi.mocked(spy).mock.lastCall[2];
        ...
        expect(vi.mocked(spy).mock.lastCall[0]).toEqual(odataQueryBatch);
        expect(vi.mocked(spy).mock.lastCall[1]).toEqual(odataQueryBatch.getBody());
        const httpOptionsArg: HttpOptions = vi.mocked(spy).mock.lastCall[2];
    ...
    it('test getBody', () => {
        ...
        vi.spyOn(odataQueryBatch, 'getUUID').mockReturnValue('');
